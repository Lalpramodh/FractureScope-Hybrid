...
On Linux, the loader reads the cgroup memory budget before importing `timm` or allocating Swin. If the worker cannot reserve `SWIN_MEMORY_RESERVE_MB` (default `460`) safely, the request keeps the YOLO detection and returns `Classification unavailable` instead of allowing the worker to be killed and returning a gateway 502.
